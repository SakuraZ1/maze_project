...
let engine = (tbl, state, buf) => {
  let result = c_engine(tbl, state, buf)
  if result >= 0 {
...
let new_engine = (tbl, state, buf) => {
  let result = c_new_engine(tbl, state, buf)
  if result >= 0 {
    buf.lex_start_p = buf.lex_curr_p
    buf.lex_curr_p = {
      ...buf.lex_curr_p,
...
let lex_refill = (read_fun, aux_buffer, lexbuf) => {
  let read = read_fun(aux_buffer, Bytes.length(aux_buffer))
  let n = if read > 0 {
    read
  } else {
 ...
 */
  if lexbuf.lex_buffer_len + n > Bytes.length(lexbuf.lex_buffer) {
    ...
    if lexbuf.lex_buffer_len - lexbuf.lex_start_pos + n <= Bytes.length(lexbuf.lex_buffer) {
      ...
      let newlen = 2 * Bytes.length(lexbuf.lex_buffer)
...
let from_string = s => {
  refill_buff: lexbuf => lexbuf.lex_eof_reached = true,
...
let lexeme = lexbuf => {
  let len = lexbuf.lex_curr_pos - lexbuf.lex_start_pos
...
let flush_input = lb => {
  lb.lex_curr_pos = 0
  lb.lex_abs_pos = 0
  lb.lex_curr_p = {...lb.lex_curr_p, pos_cnum: 0}
  lb.lex_buffer_len = 0
}
