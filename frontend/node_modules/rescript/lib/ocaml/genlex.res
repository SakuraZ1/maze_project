...
let store = c => {
  if bufpos.contents >= Bytes.length(buffer.contents) {
    let newbuffer = Bytes.create(2 * bufpos.contents)
    Bytes.blit(buffer.contents, 0, newbuffer, 0, bufpos.contents)
    buffer := newbuffer
  }
  Bytes.set(buffer.contents, bufpos.contents, c)
  incr(bufpos)
}
...
let make_lexer = keywords => {
  ...
  let ident_or_keyword = id =>
    try Hashtbl.find(kwd_table, id) catch {
    | Not_found => Ident(id)
    }
  and keyword_or_error = c => {
    let s = String.make(1, c)
    try Hashtbl.find(kwd_table, s) catch {
    | Not_found => raise(Stream.Error("Illegal character " ++ s))
    }
  }

  let rec next_token = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some(' ' | '\n' | '\r' | '\t' | '' | '') =>
      Stream.junk(strm__)
      next_token(strm__)
    | Some(('A' .. 'Z' | 'a' .. 'z' | '_' | 'À' .. 'ÿ') as c) =>
      Stream.junk(strm__)
      let s = strm__
      reset_buffer()
      store(c)
    ...
    | Some('"') =>
      ...
      let s = strm__
      reset_buffer()
      Some(String(string(s)))
    | Some('-') =>
      Stream.junk(strm__)
      neg_number(strm__)
    | Some('(') =>
      Stream.junk(strm__)
      maybe_comment(strm__)
    | Some(c) =>
      Stream.junk(strm__)
  ...
  and neg_number = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some('0' .. '9' as c) =>
      Stream.junk(strm__)
      let s = strm__
  ...
  and decimal_part = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some('0' .. '9' as c) =>
      Stream.junk(strm__)
      let s = strm__
  ...
  and exponent_part = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some(('+' | '-') as c) =>
      Stream.junk(strm__)
      let s = strm__
  ...
  and end_exponent_part = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some('0' .. '9' as c) =>
      Stream.junk(strm__)
      let s = strm__
  ...
  and char = (strm__: Stream.t<_>) =>
    switch Stream.peek(strm__) {
    | Some('\\') =>
      Stream.junk(strm__)
      try escape(strm__) catch {
      | Stream.Failure => raise(Stream.Error(""))
      }
    | Some(c) =>
      Stream.junk(strm__)
  ...
  and escape = (strm__: Stream.t<_>) =>
    ...
    | Some('0' .. '9' as c1) =>
      ...
      | Some('0' .. '9' as c2) =>
        ...
        | Some('0' .. '9' as c3) =>
          Stream.junk(strm__)
          Char.chr((Char.code(c1) - 48) * 100 + (Char.code(c2) - 48) * 10 + (Char.code(c3) - 48))
        | _ => raise(Stream.Error(""))
        }
      | _ => raise(Stream.Error(""))
      }
