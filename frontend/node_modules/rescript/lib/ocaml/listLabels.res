...
let rec fold_left = (~f, ~init as accu, l) =>
  switch l {
  | list{} => accu
  | list{a, ...l} => fold_left(~f, ~init=f(accu, a), l)
  }

let rec fold_right = (~f, l, ~init as accu) =>
  switch l {
  | list{} => accu
  | list{a, ...l} => f(a, fold_right(~f, l, ~init=accu))
  }
...
let rec fold_left2 = (~f, ~init as accu, l1, l2) =>
  switch (l1, l2) {
  | (list{}, list{}) => accu
  | (list{a1, ...l1}, list{a2, ...l2}) => fold_left2(~f, ~init=f(accu, a1, a2), l1, l2)
  | (_, _) => invalid_arg("List.fold_left2")
  }

let rec fold_right2 = (~f, l1, l2, ~init as accu) =>
  switch (l1, l2) {
  | (list{}, list{}) => accu
  | (list{a1, ...l1}, list{a2, ...l2}) => f(a1, a2, fold_right2(~f, l1, l2, ~init=accu))
...
let rec for_all = (~f as p, param) =>
  switch param {
  | list{} => true
  | list{a, ...l} => p(a) && for_all(~f=p, l)
  }

let rec exists = (~f as p, param) =>
  switch param {
  | list{} => false
  | list{a, ...l} => p(a) || exists(~f=p, l)
  }

let rec for_all2 = (~f as p, l1, l2) =>
  switch (l1, l2) {
  | (list{}, list{}) => true
  | (list{a1, ...l1}, list{a2, ...l2}) => p(a1, a2) && for_all2(~f=p, l1, l2)
  | (_, _) => invalid_arg("List.for_all2")
  }

let rec exists2 = (~f as p, l1, l2) =>
  switch (l1, l2) {
  | (list{}, list{}) => false
  | (list{a1, ...l1}, list{a2, ...l2}) => p(a1, a2) || exists2(~f=p, l1, l2)
...
let rec mem = (x, ~set) =>
  switch set {
  | list{} => false
  | list{a, ...l} => compare(a, x) == 0 || mem(x, ~set=l)
  }

let rec memq = (x, ~set) =>
  switch set {
  | list{} => false
  | list{a, ...l} => a === x || memq(x, ~set=l)
  }
...
let rec mem_assoc = (x, ~map) =>
  switch map {
  | list{} => false
  | list{(a, _), ...l} => compare(a, x) == 0 || mem_assoc(x, ~map=l)
  }

let rec mem_assq = (x, ~map) =>
  switch map {
  | list{} => false
  | list{(a, _), ...l} => a === x || mem_assq(x, ~map=l)
  }
...
let rec compare_length_with = (l, ~len as n) =>
  switch l {
  | list{} =>
    if n == 0 {
      0
