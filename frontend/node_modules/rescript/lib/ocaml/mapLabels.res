...
module Make = (Ord: OrderedType) => {
  ...
  let rec add = (~key as x, ~data, param) =>
    switch param {
  ...
  let rec update = (~key as x, ~f, param) =>
    switch param {
    | Empty =>
      switch f(None) {
      | None => Empty
  ...
  let rec fold = (~f, m, ~init as accu) =>
    switch m {
    | Empty => accu
    | Node({l, v, d, r}) => fold(~f, r, ~init=f(~key=v, ~data=d, fold(~f, l, ~init=accu)))
    }

  let rec for_all = (~f as p, param) =>
    switch param {
    | Empty => true
    | Node({l, v, d, r}) => p(v, d) && (for_all(~f=p, l) && for_all(~f=p, r))
    }

  let rec exists = (~f as p, param) =>
    switch param {
    | Empty => false
    | Node({l, v, d, r}) => p(v, d) || (exists(~f=p, l) || exists(~f=p, r))
    }
